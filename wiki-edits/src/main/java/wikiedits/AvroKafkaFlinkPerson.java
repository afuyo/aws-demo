...
import com.ippontech.kafkatutorials.Person;
...
public class AvroKafkaFlinkPerson {
    ...
    public static void main(String[] args) throws Exception {
     ...
     /**   if (parameterTool.getNumberOfParameters() < 6) {
        ...
        DataStreamSource<Person> input = env
                .addSource(
                        new FlinkKafkaConsumer011<>(
                                "persons-avro",
                                ConfluentRegistryAvroDeserializationSchema.forSpecific(Person.class, schemaRegistryUrl),
                                config).setStartFromEarliest());

        input.rebalance().print();

      SingleOutputStreamOperator<String> mapToString = input
                .map((MapFunction<Person, String>) SpecificRecordBase::toString);
